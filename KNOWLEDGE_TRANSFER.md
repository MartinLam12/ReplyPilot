...
- Fix 5 — Marketing copy audit (2026-06-05): (a) `about/page.tsx` rewrote from AI-consulting-service copy to accurate ReplyPilot description; dead CTA link to `/assessment` replaced with `/signup`. (b) `Footer.tsx`: tagline replaced ("Helping small businesses find their AI advantage…" → "AI-drafted email replies built for boxing and martial arts gym owners."), dead Product link "Take Assessment → /assessment" replaced with "Get Started → /signup", bottom tagline updated to "Built for boxing gyms…". (c) `page.tsx` landing: "Free to use" and "No credit card" hero badges removed (app requires a paid Stripe subscription); CTA body "No credit card required" removed. (d) `subscribe/page.tsx`: "Automated follow-up scheduling for leads and members" removed (feature does not exist); "Unlimited drafts and replies every month" corrected to "Daily AI reply drafts…" (daily limits are enforced via `enforceDailyLimit`).
